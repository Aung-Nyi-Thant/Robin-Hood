...
import { resolveActiveDrawPlayerId } from './gameTurn';
...
function Game({ state, socket, effects, dismissEffects }: { state: ClientGameState; socket: Socket; effects: GameEffects; dismissEffects: () => void }) {
  ...
  const activeDrawPlayerId = resolveActiveDrawPlayerId(state);
  const actionAvailable = dealAction || (!you.isSheriff && ((state.phase === 'DRAW' && activeDrawPlayerId === you.id) || (state.phase === 'BAG_SUBMIT' && !state.submittedPlayerIds.includes(you.id))));
