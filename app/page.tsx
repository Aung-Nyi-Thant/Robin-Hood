...
function Game({ state, socket, effects, dismissEffects }: { state: ClientGameState; socket: Socket; effects: GameEffects; dismissEffects: () => void }) {
  ...
  const actionAvailable = dealAction || (!you.isSheriff && ((state.phase === 'DRAW' && !state.drawCompletedPlayerIds.includes(you.id)) || (state.phase === 'BAG_SUBMIT' && !state.submittedPlayerIds.includes(you.id))));
  ...
  const takePrimaryAction = () => {
    if (state.phase === 'DEAL') { socket.emit('deal:card'); return; }
...
export default function Home() {
  ...
  const content = useMemo(() => !state ? <Welcome socket={socket} error={error}/> : state.phase === 'LOBBY' ? <Lobby state={state} socket={socket!}/> : <Game key={`${state.currentRound}-${state.phase}`} state={state} socket={socket!} effects={effects} dismissEffects={() => setEffects(dismissCoinEffects)}/>, [state, socket, error, effects]);
